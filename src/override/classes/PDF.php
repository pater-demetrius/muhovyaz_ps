...
class PDF extends PDF_PageGroup
{
	...
	private static $_fpdf_core_fonts = array('dejavusanscondensed');
...
//		FPDF::FPDF($orientation, $unit, $format);
	...
	*/
    ...
    $conf['PS_SHOP_NAME'] = isset($conf['PS_SHOP_NAME']) ? $conf['PS_SHOP_NAME'] : 'Your company';
		$conf['PS_SHOP_ADDR1'] = isset($conf['PS_SHOP_ADDR1']) ? $conf['PS_SHOP_ADDR1'] : 'Your company';
		$conf['PS_SHOP_CODE'] = isset($conf['PS_SHOP_CODE']) ? $conf['PS_SHOP_CODE'] : 'Postcode';
		$conf['PS_SHOP_CITY'] = isset($conf['PS_SHOP_CITY']) ? $conf['PS_SHOP_CITY'] : 'City';
		$conf['PS_SHOP_COUNTRY'] = isset($conf['PS_SHOP_COUNTRY']) ? $conf['PS_SHOP_COUNTRY'] : 'Country';
		$conf['PS_SHOP_STATE'] = isset($conf['PS_SHOP_STATE']) ? $conf['PS_SHOP_STATE'] : '';
...
//			$this->Cell(77, 10, self::l('DELIVERY SLIP #').' '.(Configuration::get('PS_DELIVERY_PREFIX', (int)($cookie->id_lang))).sprintf('%06d', self::$delivery), 0, 1, 'R');
			$this->Cell(77, 10, self::l('DELIVERY SLIP #').' '.Configuration::get('PS_DELIVERY_PREFIX', (int)($cookie->id_lang)).sprintf('%06d', self::$delivery), 0, 1, 'R');
		elseif (self::$order->invoice_number)
//			$this->Cell(77, 10, self::l('INVOICE #').' '.(Configuration::get('PS_INVOICE_PREFIX', (int)($cookie->id_lang))).sprintf('%06d', self::$order->invoice_number), 0, 1, 'R');
			$this->Cell(77, 10, self::l('INVOICE #').' '.Configuration::get('PS_INVOICE_PREFIX', (int)($cookie->id_lang)).sprintf('%06d', self::$order->invoice_number), 0, 1, 'R');
		else
	...
	*/
    ...
    $this->Cell(190, 5, 'P. '.$this->GroupPageNo().' / '.$this->PageGroupAlias(), 'T', 1, 'R');
		...
		if (!self::$delivery)
		{
			...
			$this->Ln(4);
//			$this->Cell(0, 10, (Configuration::get('PS_SHOP_NAME')).' '.self::l('website using your e-mail address and password (which you created when placing your first order).'), 0, 0, 'C', 0, (Configuration::get('PS_SSL_ENABLED') ? 'https://' : 'http://').$_SERVER['SERVER_NAME'].__PS_BASE_URI__.'history.php');
			$this->Cell(0, 10, Configuration::get('PS_SHOP_NAME').' '.self::l('website using your e-mail address and password (which you created when placing your first order).'), 0, 0, 'C', 0, (Configuration::get('PS_SSL_ENABLED') ? 'https://' : 'http://').$_SERVER['SERVER_NAME'].__PS_BASE_URI__.'history.php');
		}
		else
			$this->Ln(4);
...
//			$conf[$key] = ($value);
		...
		{
			$pdf->Cell($width, 10, $delivery_address->company, 0, 0, 'L');
			$pdf->Ln(5);
		}
		$pdf->Cell($width, 10, $delivery_address->firstname.' '.$delivery_address->lastname, 0, 0, 'L');
		$pdf->Ln(5);
		$pdf->Cell($width, 10, $delivery_address->address1, 0, 0, 'L');
		$pdf->Ln(5);
		if (!empty($delivery_address->address2))
		{
			$pdf->Cell($width, 10, ($delivery_address->address2), 0, 0, 'L');
			$pdf->Ln(5);
		}
		$pdf->Cell($width, 10, $delivery_address->postcode.' '.($delivery_address->city), 0, 0, 'L');
		$pdf->Ln(5);
		$pdf->Cell($width, 10, ($delivery_address->country.($deliveryState ? ' - '.$deliveryState->name : '')), 0, 0, 'L');
	...
	*/
	public function ProdReturnTab()
	{
		...
		{
			$before = $this->GetY();
			$this->MultiCell($w[0], 5, ($product['product_name']), 0, 'B');
			$lineSize = $this->GetY() - $before;
			$this->SetXY($this->GetX() + $w[0], $this->GetY() - $lineSize);
			$this->Cell($w[1], $lineSize, ($product['product_reference'] != '' ? ($product['product_reference']) : '---'), 0, 'B');
			$this->Cell($w[2], $lineSize, $product['product_quantity'],  0,  0, 'B', false, '', 0, false, 'T',  'C');
			$this->Ln();
		}
	}
	...
	*/
	public static function invoice($order, $mode = 'D', $multiple = false, &$pdf = NULL, $slip = false, $delivery = false)
	{
		...
		$pdf->Cell($width, 10, self::l('Delivery'), 0, 0, 'L');
		$pdf->Cell($width, 10, self::l('Invoicing'), 0, 0, 'L');
		$pdf->Ln(5);
		$pdf->SetFont(self::fontname(), '', 9);
		...
		{
			$pdf->Cell($width, 10, ($delivery_address->company), 0, 0, 'L');
			$pdf->Cell($width, 10, ($invoice_address->company), 0, 0, 'L');
			$pdf->Ln(5);
		}
		$pdf->Cell($width, 10, ($delivery_address->firstname).' '.($delivery_address->lastname), 0, 0, 'L');
		$pdf->Cell($width, 10, ($invoice_address->firstname).' '.($invoice_address->lastname), 0, 0, 'L');
		$pdf->Ln(5);
		$pdf->Cell($width, 10, ($delivery_address->address1), 0, 0, 'L');
		$pdf->Cell($width, 10, ($invoice_address->address1), 0, 0, 'L');
		$pdf->Ln(5);
		if (!empty($invoice_address->address2) OR !empty($delivery_address->address2))
		{
			$pdf->Cell($width, 10, ($delivery_address->address2), 0, 0, 'L');
			$pdf->Cell($width, 10, ($invoice_address->address2), 0, 0, 'L');
			$pdf->Ln(5);
		}
		$pdf->Cell($width, 10, $delivery_address->postcode.' '.($delivery_address->city), 0, 0, 'L');
		$pdf->Cell($width, 10, $invoice_address->postcode.' '.($invoice_address->city), 0, 0, 'L');
		$pdf->Ln(5);
		$pdf->Cell($width, 10, ($delivery_address->country.($deliveryState ? ' - '.$deliveryState->name : '')), 0, 0, 'L');
		$pdf->Cell($width, 10, ($invoice_address->country.($invoiceState ? ' - '.$invoiceState->name : '')), 0, 0, 'L');
		$pdf->Ln(5);
		...
		if (Configuration::get('VATNUMBER_MANAGEMENT') AND !empty($invoice_address->vat_number))
		{
			...
			$pdf->Cell($width, 10, ($vat_delivery), 0, 0, 'L');
			$pdf->Cell($width, 10, ($invoice_address->vat_number), 0, 0, 'L');
			$pdf->Ln(5);
		}

		$pdf->Cell($width, 10, $delivery_address->phone, 0, 0, 'L');
		if($invoice_customer->dni != NULL)
			$pdf->Cell($width, 10, self::l('Tax ID number:').' '.($invoice_customer->dni), 0, 0, 'L');
		if (!empty($delivery_address->phone_mobile))
		{
			$pdf->Ln(5);
			$pdf->Cell($width, 10, $delivery_address->phone_mobile, 0, 0, 'L');
		}
		...
		$pdf->Cell(70, 6, self::l('Carrier:').($order->gift ? ' '.($carrier->name) : ''), 'L');
		...
		$pdf->Cell(70, 6, ($order->gift ? self::l('Gift-wrapped order') : ($carrier->name)), 'LRB');
		...
		if (!self::$delivery)
		{
			...
			if (Configuration::get('PS_TAX') OR $order->total_products_wt != $order->total_products)
			{
				...
				$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalProductsWithoutTax'], self::$currency)), 0, 0, 'R');
				$pdf->Ln(4);

				$pdf->SetFont(self::fontname(), 'B', 8);
				$width = 165;
				$pdf->Cell($width, 0, self::l('Total products (tax incl.)').' : ', 0, 0, 'R');
				$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalProductsWithTax'], self::$currency)), 0, 0, 'R');
				$pdf->Ln(4);
			}
			else
			{
			...
			if (!self::$orderSlip AND self::$order->total_discounts != '0.00')
			{
				...
				$pdf->Cell(0, 0, (!self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(self::$order->total_discounts, self::$currency)), 0, 0, 'R');
				$pdf->Ln(4);
			}
			...
			if(isset(self::$order->total_wrapping) and ((float)(self::$order->total_wrapping) > 0))
			{
				...
				if (self::$_priceDisplayMethod == PS_TAX_EXC)
					$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['wrappingCostWithoutTax'], self::$currency)), 0, 0, 'R');
				else
					$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(self::$order->total_wrapping, self::$currency)), 0, 0, 'R');
				$pdf->Ln(4);
			}
			...
			if (self::$order->total_shipping != '0.00' AND (!self::$orderSlip OR (self::$orderSlip AND self::$orderSlip->shipping_cost)))
			{
				...
				if (self::$_priceDisplayMethod == PS_TAX_EXC)
					$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(Tools::ps_round($priceBreakDown['shippingCostWithoutTax'], 2), self::$currency)), 0, 0, 'R');
				else
					$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(self::$order->total_shipping, self::$currency)), 0, 0, 'R');
				$pdf->Ln(4);
			}
			...
			if (Configuration::get('PS_TAX') OR $order->total_products_wt != $order->total_products)
			{
				...
				$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice((self::$_priceDisplayMethod == PS_TAX_EXC ? $priceBreakDown['totalWithTax'] : $priceBreakDown['totalWithoutTax']), self::$currency)), 0, 0, 'R');
				$pdf->Ln(4);
				$pdf->Cell($width, 0, self::l('Total').' '.(self::$_priceDisplayMethod == PS_TAX_EXC ? self::l(' (tax excl.)') : self::l(' (tax incl.)')).' : ', 0, 0, 'R');
				$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice((self::$_priceDisplayMethod == PS_TAX_EXC ? $priceBreakDown['totalWithoutTax'] : $priceBreakDown['totalWithTax']), self::$currency)), 0, 0, 'R');
				$pdf->Ln(4);
			}
			else
			{
				$pdf->Cell($width, 0, self::l('Total').' : ', 0, 0, 'R');
				$pdf->Cell(0, 0, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(($priceBreakDown['totalWithoutTax']), self::$currency)), 0, 0, 'R');
				$pdf->Ln(4);
			}
			...
			if (!$delivery OR ((int)($product['product_quantity']) - (int)($product['product_quantity_refunded']) > 0))
			{
				...
				if (isset($customizedDatas[$product['product_id']][$product['product_attribute_id']]))
				{
					...
					if ($delivery)
						$this->SetX(25);
					$before = $this->GetY();
					$this->MultiCell($w[++$i], 5, ($product['product_name']).' - '.self::l('Customized'), 'B', 'L');
					$lineSize = $this->GetY() - $before;
					$this->SetXY($this->GetX() + $w[0] + ($delivery ? 15 : 0), $this->GetY() - $lineSize);
					$this->Cell($w[++$i], $lineSize, $product['product_reference'], 'B', 0, 'C');
					if (!$delivery)
						$this->Cell($w[++$i], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($unit_price, self::$currency)), 'B', 0, 'R', false, '', 0, false, 'T',  'R');
					$this->Cell($w[++$i], $lineSize, (int)($product['customizationQuantityTotal']), 'B', 0, 'C', false, '', 0, false, 'T',  'C');
					if (!$delivery)
						$this->Cell($w[++$i], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($unit_price * (int)($product['customizationQuantityTotal']), self::$currency)), 'B', 0, 'R', false, '', 0, false, 'T',  'R');
					$this->Ln();
					$i = -1;
					$total_with_tax = $unit_with_tax * $productQuantity;
					$total_without_tax = $unit_without_tax * $productQuantity;
				...
				if ($productQuantity)
				{
					...
					$lineSize = $this->GetY() - $before;
					$this->SetXY($this->GetX() + $w[0] + ($delivery ? 15 : 0), $this->GetY() - $lineSize);
					$this->Cell($w[++$i], $lineSize, ($product['product_reference'] ? $product['product_reference'] : '--'), 'B', 0, 'C');
					if (!$delivery)
						$this->Cell($w[++$i], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($unit_price, self::$currency)), 'B', 0, 'R', false, '', 0, false, 'T',  'R');
					$this->Cell($w[++$i], $lineSize, $productQuantity, 'B', 0, 'C', false, '', 0, false, 'T',  'C');
					if (!$delivery)
						$this->Cell($w[++$i], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($final_price, self::$currency)), 'B', 0, 'R', false, '', 0, false, 'T',  'R');
					$this->Ln();
				}
			}
...
g	* Discount table with value, quantities...
	*/
	public function DiscTab()
	{
		...
		{
			$this->Cell($w[0], 6, self::l('Discount:').' '.$discount['name'], 0, 'B');
			$this->Cell($w[1], 6, '', 0, 'B');
			$this->Cell($w[2], 6, '', 0, 'B');
			$this->Cell($w[3], 6, '',  0,  0, 'B', false, '', 0, false, 'T',  'R');
			$this->Cell($w[4], 6, '1',  0,  0, 'B', false, '', 0, false, 'T',  'C');
			$this->Cell($w[5], 6, ((!self::$orderSlip AND $discount['value'] != 0.00) ? '-' : '').self::convertSign(Tools::displayPrice($discount['value'], self::$currency)),  0,  0, 'B', false, '', 0, false, 'T',  'R');
			$this->Ln();
		}
	...
	*/
	public function TaxTab(array &$priceBreakDown)
	{
		...
		{
			$this->Ln();
			$this->Cell(30, 0, self::l('Exempt of VAT according section 259B of the General Tax Code.'), 0, 0, 0, 'L');
			return;
		}
		...
		foreach ($priceBreakDown['taxes'] AS $tax_rate => $vat)
		{
			if ($tax_rate != '0.00' AND $priceBreakDown['totalsProductsWithTax'][$tax_rate] != '0.00')
			{
				...
				$this->Cell($w[2], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalsProductsWithoutTax'][$tax_rate], self::$currency)), 0, 0, 'R');
				$this->Cell($w[3], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalsProductsWithTax'][$tax_rate] - $priceBreakDown['totalsProductsWithoutTax'][$tax_rate] - $priceBreakDown['totalsEcotax'][$tax_rate], self::$currency)), 0, 0, 'R');
				if ($priceBreakDown['hasEcotax'])
					$this->Cell($w[4], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalsEcotax'][$tax_rate], self::$currency)), 0, 0, 'R');
				$this->Cell($w[$priceBreakDown['hasEcotax'] ? 5 : 4], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['totalsProductsWithTax'][$tax_rate], self::$currency)), 0, 0, 'R');
				$this->Ln();
			}
		}
		...
		if ($carrier_tax_rate AND $carrier_tax_rate != '0.00' AND ((self::$order->total_shipping != '0.00' AND !self::$orderSlip) OR (self::$orderSlip AND self::$orderSlip->shipping_cost)))
		{
			...
			$this->Cell($w[2], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['shippingCostWithoutTax'], self::$currency)), 0, 0, 'R');
			$this->Cell($w[3], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(self::$order->total_shipping - $priceBreakDown['shippingCostWithoutTax'], self::$currency)), 0, 0, 'R');
			if ($priceBreakDown['hasEcotax'])
				$this->Cell($w[4], $lineSize, (self::$orderSlip ? '-' : '').'', 0, 0, 'R');
			$this->Cell($w[$priceBreakDown['hasEcotax'] ? 5 : 4], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(self::$order->total_shipping, self::$currency)), 0, 0, 'R');
			$this->Ln();
		}
		...
		if (self::$order->total_wrapping AND self::$order->total_wrapping != '0.00')
		{
			...
			$this->Cell($w[2], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice($priceBreakDown['wrappingCostWithoutTax'], self::$currency)), 0, 0, 'R');
			$this->Cell($w[3], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(self::$order->total_wrapping - $priceBreakDown['wrappingCostWithoutTax'], self::$currency)), 0, 0, 'R');
			$this->Cell($w[4], $lineSize, (self::$orderSlip ? '-' : '').self::convertSign(Tools::displayPrice(self::$order->total_wrapping, self::$currency)), 0, 0, 'R');
		}
	...
	{
		return $s;//str_replace('¥', chr(165), str_replace('£', chr(163), str_replace('€', chr(128), $s)));
	}
