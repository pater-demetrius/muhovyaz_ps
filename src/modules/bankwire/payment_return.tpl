...
{if $status == 'ok'}
	<p>{l s='Your order on' mod='bankwire'} <span class="bold">{$shop_name}</span> {l s='is complete.' mod='bankwire'}
        ...
        {*Dimyan*}
        <br /><br />- {l s='to this bank' mod='bankwire'} <span class="bold">{if $bankwireAddress}<p style="padding-left: 30px;" class="bold">{$bankwireAddress}</p>{else}___________{/if}</span>
		<br /><br />- {l s='with these details' mod='bankwire'} <span class="bold">{if $bankwireDetails}<p style="padding-left: 30px;" class="bold">{$bankwireDetails}</p>{else}___________{/if}</span>
